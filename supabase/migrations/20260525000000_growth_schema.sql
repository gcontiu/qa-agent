-- Growth module schema — beta funnel (waitlist, drip, NPS, counters)
-- Isolated under 'growth' schema; cross-schema refs are loose TEXT user_id only.
...
-- RLS: growth tables are admin-only; no public/authenticated access
ALTER TABLE growth.waitlist ENABLE ROW LEVEL SECURITY;
ALTER TABLE growth.beta_enrollments ENABLE ROW LEVEL SECURITY;
ALTER TABLE growth.drip_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE growth.nps_responses ENABLE ROW LEVEL SECURITY;
ALTER TABLE growth.daily_counters ENABLE ROW LEVEL SECURITY;
