    ...
    <div className="p-8 max-w-4xl">
      ...
      {task && (
        <div className="mt-6">
          ...
          {task.status === 'running' && (
            <p className="text-sm text-muted-foreground">Scanning and generating specs…</p>
          )}
          {task.status === 'done' && (
      ...
      {specs.length > 0 && (
        <div className="mb-4 rounded-lg border border-border bg-muted/30 px-4 py-3 text-xs text-muted-foreground space-y-1">
          <p><span className="font-medium text-foreground">draft</span> — generated by the analyst, not yet reviewed. Open the file to edit or approve it.</p>
          <p><span className="font-medium text-foreground">approved</span> — reviewed and ready to execute. Only approved specs are included in a run.</p>
          <p><span className="font-medium text-foreground">config.yaml</span> — product metadata (base URL, environment, context for the executor). Not a test file and not subject to approval.</p>
        </div>
      )}
